			...
			if(dobleAry_index.length == 2) {
				if(dobleAry_Num[0] == dobleAry_Num[1]&&dobleAry_index[0]!=dobleAry_index[1]) {
					...
					if(allGrade == 12) {
						...
						setTimeout(function() {
							var newItem = "<div class='endingWrapper'></div>";
							$(".card_box").after(newItem);
							$(".cornerWrapper").css("display", "flex");
							$(".card_box").remove();
							$(".tipsWrapper").remove();
							$("body").css("background-image", "none");
							$("body").css("background-color", "#595474");
							$.ajax({
								type: "POST",
							...
							$.ajax({
								...
								dataType: "text",
								data: {telephone: $telephone, id: "adventureYDW", progress: 100.00, addScore: 3},  
								error: function() {
									alert("saveError");
								}
							});
